...
pro miro,input,dir_out=dir_out,res=res,n0=n0,rc=rc,bmean=bmean,kin=kin,kmax=kmax,alphak=alphak,seed=seed,name=name,ncentre=ncentre,alphaB=alphaB,nclump=nclump,rc_fila=rc_fila,nc_fila=nc_fila,write_disk=write_disk
  ...
  CPU,TPOOL_NTHREADS=8
   ...
   if not IS_DEF(dir_out) then dir_out='/Users/francovazza/Desktop/IDL_prog/basic_miro/miro_2018/out/'   ;...folder for outputs
...
kk=indgen(nk(1))+1
set_plot,'ps'
device,filename=dir_out+'spec_'+name+'.eps',xsize=16,ysize=14,/color,font_size=7,/encapsul
loadct,13
!p.font=1
!p.multi=[0,2,2]
...
plot,xb,h1,linestyle=0,xtitle='B-field[muG]',ytitle='dN/dB',title='distribution functions for 3 components',yrange=[1,max(h1)],/ystyle,psym=10
